#!/usr/bin/env python3
"""Migrate files from Supabase Storage to Azure Blob Storage (PARALLEL VERSION).

This script:
1. Reads exported books.json for file paths
2. Downloads files from Supabase Storage (books, thumbnails) in parallel
3. Uploads to Azure Blob Storage in parallel
4. Updates Azure PostgreSQL with new file paths

Usage:
    python scripts/migrate_files_parallel.py
    python scripts/migrate_files_parallel.py --dry-run
    python scripts/migrate_files_parallel.py --workers 10
"""
...
class FileStats:
    ...
    def inc_books_downloaded(self):
        with self._lock:
            self.books_downloaded += 1

    def inc_books_uploaded(self):
        with self._lock:
            self.books_uploaded += 1

    def inc_thumbnails_downloaded(self):
        with self._lock:
            self.thumbnails_downloaded += 1

    def inc_thumbnails_uploaded(self):
        with self._lock:
            self.thumbnails_uploaded += 1

    def add_error(self, error: str):
        with self._lock:
            self.errors.append(error)

    def add_file_update(self, update: dict):
        with self._lock:
            self.file_updates.append(update)
...
def process_book(
    ...
    """Process a single book - download and upload files."""
    book_id = book.get("id")
    ...
    if file_path:
        try:
            ...
            stats.add_error(f"Book {book_id}: {error_msg}")
    ...
    if thumbnail_path:
        try:
            ...
            stats.inc_thumbnails_downloaded()
            ...
            if len(error_msg) > 100:
                error_msg = error_msg[:100] + "..."
            stats.add_error(f"Thumbnail {book_id}: {error_msg}")

    # Track updates for database
    if file_update["file_path"] or file_update["cover_url"]:
        stats.add_file_update(file_update)
...
def download_and_upload_files(
    ...
    """Download files from Supabase and upload to Azure using thread pool."""
    stats = FileStats()

    print(f"Processing {len(books)} books with {max_workers} workers")
    ...
    with tqdm(total=len(books), desc="Books") as pbar:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in futures:
                try:
                    ...
                    stats.add_error(f"Worker error: {str(e)[:100]}")
...
async def main():
    parser = argparse.ArgumentParser(
        description="Migrate files from Supabase to Azure Blob Storage (parallel)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # Run the parallel file migration
    stats = download_and_upload_files(
        books,
